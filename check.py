...
Stream = namedtuple('Stream', ['name', 'game_id', 'user_id', 'title', 'thumbnail_url', 'type', 'viewer_count'])
...
def get_user(username):
	...
	r = requests.get("https://api.twitch.tv/helix/users", params = payload, headers=headers)
	return r.json()
...
def get_follows(user_id):
	...
	while still_paginated:
		r = requests.get("https://api.twitch.tv/helix/users/follows", params = payload, headers=headers)
		print(r.status_code)
...
def request_streams_info(ids):
	...
	for grouping in group_by_100s(ids):
		...
		r = requests.get("https://api.twitch.tv/helix/streams", params = payload, headers=headers)
		...
		for i in resp["data"]:
			# name =  i["display_name"]
			s = Stream(None, i["game_id"], i["user_id"], i["title"], i["thumbnail_url"], i["type"], i["viewer_count"])
			yield s
